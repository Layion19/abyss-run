...
// Progression de difficulté, mines "portes" + explosives, brume PNG,
// séparation Desktop/Mobile, Safari ok, et re-scale complet du monde.
// ==========================================================================
...
(function () {
  ...
  // ---------- Canvas responsive (avec re-scale du monde) ----------
  ...
  // --- ÉTAT & ENTITÉS (déclarés ici pour que rescaleWorld y ait accès) ---
  const orbs=[], mines=[], orcas=[], hearts=[];
  const explosions=[], flashes=[], bubbles=[], orcaBubbles=[];
  const fogSprites=[];
  let fogSpawnBucket=0, fogSeqIndex=0, waterU=0;
  ...
  const state = {
    ...
    // timers
    orbTimer: 0.6, mineTimer: 0.9, orcaTimer: 2.8,
  ...
  // ---------- Re-scale intégral des entités lors d’un resize ----------
  function rescaleWorld(kx, ky){
    ...
    // Brume
    for (const f of fogSprites){ f.x *= kx; f.y *= ky; f.w *= kx; f.h *= ky; }
  }
  ...
  function frameSize(){
    ...
    // borné par la hauteur de viewport (petite marge ergonomique)
  ...
  function resizeCanvasToFrame(){
    ...
    // Taille interne (unités logiques du jeu)
  ...
  const audio = {
    ...
    orb: loadAudio(["static/abyss/sfx/orb.wav","static/abyss/sfx/orb.mp3"], {volume:0.7}),
  ...
  function startRun(){ state.running = true; try { if (musicEnabled) audio.bgm.play(); } catch {} }
  function resetGame(){
  ...
  function drawWater(dt){
    ...
    if (!waterReady || !strip){
      ...
      ctx.fillStyle=g; ctx.fillRect(0,0,cw,ch); return;
    }
    const iw=waterTex.width, ih=waterTex.height;
  ...
  function spawnMine(){
    ...
    const m = { x: W()+60, y: rand(60,H()-60), w: s, h: s, explosive: state.rng() < explosiveChance(), blinking:false, blinkCount:0, blinkTimer:0, exploded:false };
    mines.push(m);
  }
  function spawnMineGate(){
    const h = H(), w = W();
    const baseSize = 24*(W()/820);
    const rowStep = 64*(h/720), margin = 48*(h/720);
    const minGap = 120*(h/720), gapGrow = 60*(1 - difficulty())*(h/720);
    ...
    for (let i=0;i<=rows;i++){
      ...
      const explosive = state.rng() < (explosiveChance() * 0.65);
  ...
  const fogTextures = [];
  (function loadFogTextures(){ for (const name of FOG_FILES){ const img = new Image(); img.src = FOG_PATH + name; fogTextures.push(img); } })();
  const FOG_CAP = 10; const fogSeq=[0,1,2,3];

  function fogMixLevel(){ const start=1000,end=2500; return clamp((state.score - start)/(end - start), 0, 1); }
  function spawnFogPhase1IfNeeded(){
    if (fogSprites.length > 0) return;
    const scale = W()/820;
    const texIndex = fogSeq[fogSeqIndex % fogSeq.length]; fogSeqIndex++;
    const tex = fogTextures[texIndex];
    const r = 220 * scale, w=r*2, h=r*2;
    const vx = -worldSpeed() * 0.26, vy = (Math.random()*10 - 5);
    const y  = Math.min(H()-60, Math.max(60, Math.random()*(H()-120)+60));
    const life = 8.5 + Math.random()*1.5;
    fogSprites.push({ x:W()+w*0.5, y, w, h, vx, vy, tex, life:0, maxLife:life, alphaMul:0.14, twist: Math.random()*Math.PI*2 });
  }
  function spawnFogPhase2(dt){
    const lvl = fogMixLevel(); if (lvl <= 0) return;
    const rate = 0.8 + 6.0*lvl; fogSpawnBucket += dt * rate;
    const scale = W()/820, darkWeight = 0.15 + 0.70*lvl;
    while (fogSpawnBucket >= 1){
      fogSpawnBucket -= 1; if (fogSprites.length >= FOG_CAP) break;
      let texIndex; if (Math.random() < darkWeight) texIndex = 4 + Math.floor(Math.random()*2); else texIndex = Math.floor(Math.random()*4);
      const tex = fogTextures[texIndex];
      const base = (texIndex>=4 ? 260 : 200); const r = (base + 180*lvl) * scale; const w=r*2, h=r*2;
      const vx = -worldSpeed() * (0.26 + 0.22*lvl) + (Math.random()*24 - 12);
      const vy = (Math.random()*16 - 8);
      const y  = Math.min(H()-40, Math.max(40, Math.random()*(H()-80)+40));
      const life = 7.5 + 6*lvl + (Math.random()*1.5-0.75);
      fogSprites.push({ x:W()+w*0.5, y, w, h, vx, vy, tex, life:0, maxLife:life, alphaMul:0.16 + 0.22*lvl, twist: Math.random()*Math.PI*2 });
    }
  }
  function updateFog(dt){
    for (let i=fogSprites.length-1;i>=0;i--){
      const f = fogSprites[i];
      f.life += dt; f.x += f.vx * dt; f.y += f.vy * dt + Math.sin(state.elapsed*0.5 + f.twist) * 3.0 * dt;
  ...
  function drawFog(){
    ...
    for (const f of fogSprites){
      const t = clamp(f.life / f.maxLife, 0, 1), a = Math.sin(Math.PI * t), alpha = f.alphaMul * a;
      if (f.tex && f.tex.complete){
        ctx.globalAlpha = alpha; ctx.save(); ctx.translate(f.x, f.y);
        ctx.rotate(Math.sin((f.twist + state.elapsed)*0.15) * 0.08);
        ctx.drawImage(f.tex, -f.w/2, -f.h/2, f.w, f.h); ctx.restore();
      }
    }
  ...
  function update(dt){
    ...
    if (state.orbTimer <= 0){ spawnOrb(); state.orbTimer  = (Math.random()*(1.15-0.80)+0.80) / spawnScale; }
    if (state.mineTimer <= 0){ spawnMine(); state.mineTimer = (Math.random()*(1.55-0.95)+0.95) / (spawnScale * (1 + d*0.25)); }
    if ((state.score>160 || state.elapsed>28) && state.orcaTimer <= 0){
      spawnOrca(); state.orcaTimer = (Math.random()*(6.4-4.0)+4.0) / (0.85 + 0.55*spawnScale);
    }
    ...
    for (let i=orcas.length-1;i>=0;i--){
      ...
      if (circleHit(player.x(),player.y,player.radius, o.x+o.w*0.5, o.y+o.h*0.5, rr)){ orcas.splice(i,1); takeHit(); }
    }
    for (let i=hearts.length-1;i>=0;i--){
      const h=hearts[i]; const cx = (h.x!==undefined ? h.x : (h.x = W()+70));
      if (circleHit(player.x(),player.y,player.radius, cx + (h.s*0.5), h.y + (h.s*0.5), Math.max(18, h.s*0.35))){
        hearts.splice(i,1); state.lives = Math.min(6, state.lives + 1); playSfx(audio.heart, 0.9);
      }
    }

    // Brume
    if (state.score < 1000){ spawnFogPhase1IfNeeded(); } else { spawnFogPhase2(dt); }
    updateFog(dt);
  ...
  function takeHit(){
    ...
    if (state.lives<=0) { state.running=false; submitBestRun(); showGameOverOverlay(); }
  }
  ...
  function drawOrb(o){
    const pulse = 0.7 + 0.3*Math.sin(o.phase||0), glowR = (o.r*2.1) * pulse;
    ctx.save(); ctx.globalCompositeOperation="screen";
    ...
    if (sprites.orb.ready) { const s=50*(W()/820) * (0.95 + 0.05*Math.sin((o.phase||0)*1.5)); ctx.drawImage(sprites.orb.img, o.x-s/2, o.y-s/2, s, s); }
    else { ctx.beginPath(); ctx.arc(o.x,o.y,o.r,0,Math.PI*2); ctx.fillStyle="rgba(180,255,200,0.95)"; ctx.fill(); ctx.beginPath(); ctx.arc(o.x-o.r*.3,o.y-o.r*.3,o.r*.4,0,Math.PI*2); ctx.fillStyle="rgba(255,255,255,0.85)"; ctx.fill(); }
  }
  function drawMine(m){
